...
if data_integrity_issues:
    st.warning(f"ÔÜá´©Å Found {len(data_integrity_issues)} data integrity issues")
    for issue in data_integrity_issues[:5]:  # Show first 5 issues
        st.write(f"ÔÇó {issue}")
    
    # Export functionality
    if st.button("­ƒôè Export Anomaly Data to CSV"):
        anomaly_data = []
        ...
        if anomaly_data:
            ...
            st.download_button(
                label="­ƒôÑ Download Anomaly Report (CSV)",
                data=csv,
